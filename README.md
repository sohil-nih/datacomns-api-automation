...
|---------------|---------|
| **Federation OpenAPI contract** (autogenerated GETs + HTML/JSON) | `bash scripts/run_federation_contract_suite.sh` or `python -m framework.contract_runner.federation_cli` |
...
│   │   ├── dcc_cli.py        # DCC contract CLI
│   │   ├── federation_cli.py # Federation contract CLI
│   │   ├── dcc_perf_cli.py   # DCC concurrent perf CLI
│   │   ├── dcc_discover.py   # live API discovery (DCC)
│   │   ├── federation_discover.py  # Federation discovery (delegates to DCC when shapes match)
│   │   ├── dcc_generator.py  # DCC OpenAPI → case dicts
│   │   ├── federation_generator.py  # Federation paths (/subject, …)
