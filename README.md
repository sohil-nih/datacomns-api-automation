...
| Area | Key vars |
|------|----------|
| Federation | `DATACOMNS_FEDERATION_BASE_URL`, `MEMGRAPH_URI`, `MEMGRAPH_USER`, `MEMGRAPH_PASSWORD`; optional `FEDERATION_CONTRACT_REPORT_DIR`, `FEDERATION_CONTRACT_EXCLUDE_PATHS`; `FEDERATION_AL_EXPECTED_SOURCES` overrides default seven-node AL roster (use `none` or `-` to disable) |
