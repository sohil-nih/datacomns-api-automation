"""
CLI: DCC OpenAPI-driven contract tests (discover → generate → run GET cases → reports).

Uses ``config/projects.yaml`` entry ``dcc`` for base URL (``DATACOMNS_DCC_BASE_URL``),
``api_prefix``, headers, and OpenAPI path.

Environment:
  DCC_CONTRACT_REPORT_DIR — default ``reports/dcc/contract``
...
def main_dcc() -> None:
    ...
    parser = argparse.ArgumentParser(description="DCC API OpenAPI contract test runner")
    parser.add_argument(
        "--spec",
        default=None,
        help="OpenAPI JSON/YAML (default: path from config/projects.yaml dcc.openapi_relative)",
    )
    parser.add_argument("--base-url", default=None, help="Override API root (normally from project config)")
    parser.add_argument(
        "--report",
        default=None,
        help="Report directory (default: DCC_CONTRACT_REPORT_DIR or reports/dcc/contract)",
    )
    ...
    report_dir = args.report or os.getenv("DCC_CONTRACT_REPORT_DIR", "reports/dcc/contract")
    try:
    ...
    json_path = Path(report_dir) / f"dcc_contract_{run_id}.json"
    html_path = Path(report_dir) / f"dcc_contract_{run_id}.html"
