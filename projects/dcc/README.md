...
- **OpenAPI contract suite (autogenerated):** `projects/dcc/contracts/openapi.json` — run `bash scripts/run_dcc_contract_suite.sh` or `python -m framework.contract_runner.dcc_cli` (reports under `reports/dcc/contract/`). Discovery reads `/subject`, `/sample`, and `/file` list payloads and fills `filter_examples` so the generator adds up to **20** extra GETs per list route (one query param each, values taken from live `metadata`). Use `--strict-filter-data` to require a non-empty `data` array on those filter cases.
